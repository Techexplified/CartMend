...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    const reason = body.reason || "CUSTOMER";
    let shopDomain =
      body.shopDomain ||
      body.shop ||
    ...
    const result = await PostPurchaseActionService.cancelOrder(shopDomain, shopifyOrderId, reason);
    ...
    console.error("[CartMend] post-purchase/cancel error:", error);
    return Response.json(
