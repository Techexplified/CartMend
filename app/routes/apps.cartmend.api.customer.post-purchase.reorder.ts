...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    return Response.json(
      {
        success: true,
        cartUrl: reorderResult.cartUrl,
        itemsCount: reorderResult.itemsCount,
        unavailableItems: reorderResult.unavailableItems,
      },
      {
        status: 200,
    ...
    console.error("[CartMend App Proxy Post-Purchase Reorder Error]:", error);
    return Response.json(
      { error: error.message || "Failed to generate reorder cart" },
